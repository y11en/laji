    ...
    <div class="breadcrumb-wrap">
        ...
        <div>
            <el-breadcrumb separator-class="el-icon-arrow-right">
                ...
                <el-breadcrumb-item>
                    <template v-if="$route.name=='addChapter'">新增章节</template>
                    <template v-else="">编辑章节</template>
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
    </div>
            ...
            <el-form-item label="章节内容" class="contentBox" prop="chapterContent">
                ...
                <el-form-item prop="authorWords">
                    <div class="author-say-wrap">
                        <el-input class="author-say" type="textarea" v-model="ruleForm.authorWords" placeholder="作者的话（不可超过200字）"></el-input>
  ...
  export default {
            ...
            service.FetchNetTime().then(time=>{
                if(time.returnCode===200){
                    ...
                    service.FetchAuthorHandleBook(cloneData,'ac').then(json=>{
                        ...
                        }else this.$message( { message: json.msg, type: 'error' } )
                    })
                }else {
                    this.$nextTick(()=>{
                        this.$loading().close()
    ...
    watch: {
        ...
        "ruleForm.authorWords": function(val) {
            // if(val.length >200){
            //     this.ruleForm.authorWords = val.slice(0, 199)
            // }
